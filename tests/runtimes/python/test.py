...
class TestEdgeCase:
    def test_t301_not_a_comment(self, lex):
        assert lex.get("not_a_comment") == "#not a comment"

    def test_t302_double_colon(self, lex):
        assert lex.get(
            "double_colon"
        ) == "key::value with double colon in value"

    def test_t303_triple_colon(self, lex):
        assert lex.get(
            "triple_colon"
...
class TestConformance:
    def test_t401_missing_key_raises(self, lex):
        with pytest.raises(LexKeyNotFoundError):
            lex.get("nonexistent_key")

    def test_t402_missing_key_with_default(self, lex):
        assert lex.get_or_default("nonexistent_key", "fallback") == "fallback"

    def test_t403_missing_key_with_default_and_args(self, lex):
    ...
    def test_t406_reload_switches_locale(self, lex_es):
    ...
    def test_t408_reload_rollback_preserves_fallback(self, lex_en):
    ...
    def test_t414_lazy_caching_no_reprocess(self, lex):
